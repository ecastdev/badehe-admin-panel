...
export default function Network(){
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const dbVal = await getDocs(collection(badehestore, "network"));
    ...
    useEffect(() => {
        const fetchImages = async () => {
            try {
                ...
                const urls = await Promise.network(images.items.map(async (item) => {
    ...
    const handleCreate = async () => {
        try {
            ...
            if (confirmCreate.isConfirmed) {
                await addDoc(collection(badehestore, "network"), { Btitle: fname, Bsubtitle: lname, thePicture: imageUrl });
                setFname("");
                setLname("");
                setImgUrl('');
            }
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (confirmDELETE.isConfirmed) {
                ...
                // Delete the document
                await deleteDoc(doc(badehestore, "network", id));
            }
        } catch (error) {
    ...
    const handleUpdate = async () => {
        try {
            ...
            if (confirmUpdate.isConfirmed) {
                ...
                const updateData = doc(badehestore, "network", id);
    ...
    return(
        <div className="grid place-content-center items-center mt-5">
            ...
            <h3 className="text-4xl font-bold text-center  mt-4  ">Network installation blogs</h3>
            {/* file upload */}
