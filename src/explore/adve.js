...
export default function Adve(){
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const dbVal = await getDocs(collection(badehestore, "adve"));
    ...
    useEffect(() => {
        const fetchImages = async () => {
            try {
                const images = await listAll(ref(imageDb, "adve"));
    ...
    const handleCreate = async () => {
        try {
            ...
            let imageUrl = '';
            if (img) {
                const imgRef = ref(imageDb, `adve/${v4()}`);
            ...
            if (confirmCreate.isConfirmed) {
                await addDoc(collection(badehestore, "adve"), { Btitle: fname, Bsubtitle: lname, thePicture: imageUrl });
                setFname("");
                setLname("");
                setImgUrl('');
            }
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (confirmDELETE.isConfirmed) {
                ...
                // Delete the document
                await deleteDoc(doc(badehestore, "adve", id));
            }
        } catch (error) {
    ...
    const handleUpdate = async () => {
        try {
            ...
            if (confirmUpdate.isConfirmed) {
                ...
                const updateData = doc(badehestore, "adve", id);
    ...
    // const handleUpdate = async () => {
    //     try {
    //         const updateData = doc(badehestore, "all", id);
    //         await updateDoc(updateData, { Btitle: fname, Bsubtitle: lname, thePicture: imgUrl });
    //         setShow(false);
    //         setFname("");
    //         setLname("");
    ...
    return(
        <div className="grid place-content-center items-center mt-5">
            ...
            <h3 className="text-4xl font-bold text-center  mt-4  ">advertisement blogs</h3>
            {/* file upload */}
